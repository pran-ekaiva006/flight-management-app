 ...
 */
export async function fetchAlternativeFlights(bookingId: string): Promise<{
  flights: Array<{
    id: string;
    flight_no: string;
    departs_at: string;
  ...
  const { data: altFlights } = await supabase
    .from('flights')
    .select(
      'id, flight_no, departs_at, arrives_at, base_price, aircraft_type, seats(id, is_available)',
    )
  ...
  const flights = (altFlights || [])
    .map((f) => {
      const seats =
        (f.seats as Array<{ id: string; is_available: boolean }>) || [];
      return {
        id: f.id,
...
export async function rescheduleBookingAction(
  ...
  if (error) {
    ...
    if (msg.includes('ROUTE_MISMATCH'))
      return { success: false, error: 'New flight must be on the same route.' };
    if (msg.includes('SAME_FLIGHT'))
      return { success: false, error: 'Cannot reschedule to the same flight.' };
    if (msg.includes('SEAT_TAKEN'))
      return { success: false, error: 'Selected seat is no longer available.' };
    if (msg.includes('INVALID_STATUS'))
      return {
        success: false,
        error: 'Only confirmed bookings can be rescheduled.',
      };
    if (msg.includes('FLIGHT_DEPARTED'))
      return { success: false, error: 'New flight has already departed.' };
